...
# Header principal
st.markdown('<div class="main-title">🎮 Sistema IoT Inteligente</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Control por voz y detección de colores</div>', unsafe_allow_html=True)
                ...
                if response.choices[0].message.content:
                    ...
                    try:
                        ...
                        if colors_found:
                            ...
                            # ENCENDER LEDs AUTOMÁTICAMENTE según colores detectados
                            try:
                                client2 = paho.Client("color-detection-control")
                                ...
                                if color_data.get("amarillo"):
                                    commands_to_send.append("enciende amarillo")
                                if color_data.get("verde"):
                                    commands_to_send.append("enciende verde")
                                if color_data.get("rojo"):
                                    commands_to_send.append("enciende rojo")
                                
                                # Enviar comandos
                                for cmd in commands_to_send:
                                    message = json.dumps({"Act1": cmd})
                                    ret = client2.publish("appcolor", message)
                                    st.toast(f"💡 Encendiendo LED: {cmd}", icon="✅")
                                    time.sleep(0.5)  # Pequeña pausa entre comandos
                                
                                client2.disconnect()
                                st.success("🎛️ LEDs encendidos automáticamente según colores detectados")
                                
                            except Exception as e:
                                st.error(f"❌ Error al enviar comandos de LED: {e}")
